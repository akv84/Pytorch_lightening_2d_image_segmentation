...
class BinaryMetrics():
    ...
    def _calculate_count_ratio(self, gt, pred):
        gt_1 = gt.view(gt.shape[0], -1)
        pred_1 = pred.view(pred.shape[0], -1)
        gt_sum = torch.max(gt_1, axis=1).values
        pred_sum = torch.max(pred_1, axis=1).values
        pred_sum = pred_sum*gt_sum
        pred_sum = pred_sum.sum()
        gt_sum = gt_sum.sum()
        
        return (pred_sum+self.eps)/(gt_sum+self.eps)
    ...
    def __call__(self, y_true, y_pred):
        ...
        pixel_acc, dice, precision, specificity, recall = self._calculate_overlap_metrics(y_true.to(y_pred.device, dtype=torch.float), activated_pred)
        count_ratio = self._calculate_count_ratio(y_true.to(y_pred.device, dtype=torch.float), activated_pred)
        return [pixel_acc, dice, precision, specificity, recall, count_ratio]
